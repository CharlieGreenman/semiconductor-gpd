...
tools: read_file, write_file, shell, glob, grep, web_search, web_fetch
...
<references>
- `@./.codex/get-physics-done/references/verification/core/verification-core.md` -- Universal verification checks and priority patterns
- `@./.codex/get-physics-done/references/physics-subfields.md` -- Methods, tools, and validation strategies per physics subfield
- `@./.codex/get-physics-done/references/verification/errors/llm-physics-errors.md` -- Common LLM physics errors to check against
...
for plan in "$PHASE_DIR"/*-PLAN.md; do
  echo "=== $plan ==="
  PLAN_STRUCTURE=$(/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local verify plan-structure "$plan")
  echo "$PLAN_STRUCTURE"
done
```
...
```bash
PLAN_CONTRACT=$(/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local frontmatter get "$PLAN_PATH" --field contract)
```
...
```bash
PLAN_STRUCTURE=$(/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local verify plan-structure "$PLAN_PATH")
```
