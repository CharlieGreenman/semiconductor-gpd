...
```bash
TRANSITION=$(/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local phase complete "${CURRENT_PHASE}")
if [ $? -ne 0 ]; then
...
```bash
PROGRESS=$(/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local --raw progress bar)
```
...
/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local --raw state snapshot > /dev/null
if [ $? -ne 0 ]; then
...
PRE_CHECK=$(/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local pre-commit-check --files .gpd/ROADMAP.md .gpd/STATE.md .gpd/PROJECT.md 2>&1) || true
echo "$PRE_CHECK"
```
...
```bash
/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local commit "docs(phase-${CURRENT_PHASE}): transition to next phase" --files .gpd/ROADMAP.md .gpd/STATE.md .gpd/PROJECT.md .gpd/DECISIONS.md
```
...
```bash
AUTO_COMPACT=$(/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local state compact 2>&1)
```
...
if echo "$AUTO_COMPACT" | grep -q '"compacted": true'; then
  ...
  /Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local commit "chore: auto-compact state after phase-${CURRENT_PHASE} transition" --files .gpd/STATE.md .gpd/STATE-ARCHIVE.md .gpd/state.json
fi
```
...
/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local commit "docs(phase-${CURRENT_PHASE}): autofill derivation state" --files .gpd/DERIVATION-STATE.md
```
...
```bash
PRE_CHECK=$(/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local pre-commit-check --files .gpd/STATE.md 2>&1) || true
echo "$PRE_CHECK"

/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local commit "chore: update session continuity after phase-${CURRENT_PHASE} transition" --files .gpd/STATE.md
```
