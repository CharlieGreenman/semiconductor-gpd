...
- ./.codex/get-physics-done/references/execution/git-integration.md
- ./.codex/get-physics-done/references/execution/execute-plan-recovery.md
- ./.codex/get-physics-done/references/execution/execute-plan-validation.md
- ./.codex/get-physics-done/references/execution/execute-plan-checkpoints.md
- ./.codex/get-physics-done/references/protocols/reproducibility.md
- ./.codex/get-physics-done/references/protocols/error-propagation-protocol.md -- Cross-phase uncertainty propagation protocol (Uncertainty Budget declaration format, verification checks, phase handoff format)
- ./.codex/get-physics-done/references/execution/executor-index.md -- Maps execution scenarios (QFT, condensed matter, numerical, paper writing, debugging) to the correct domain-specific reference files
- ./.codex/get-physics-done/templates/calculation-log.md -- Template for CALCULATION_LOG.md (detailed derivation records within a phase)
- ./.codex/get-physics-done/templates/recovery-plan.md -- Template for RECOVERY.md (structured recovery after plan execution failure)
</required_reading>
...
```bash
INIT=$(/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local init execute-phase "${phase}" --include state,config)
if [ $? -ne 0 ]; then
...
```bash
STATE_CONTENT=$(echo "$INIT" | /Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local json get .state_content --default "")
CONFIG_CONTENT=$(echo "$INIT" | /Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local json get .config_content --default "")
```
...
```bash
CONV_CHECK=$(/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local --raw convention check)
if [ $? -ne 0 ]; then
...
```bash
CONVENTIONS=$(/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local --raw convention list 2>/dev/null)
```
...
```bash
/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local observe event workflow execute-plan.start --phase "${phase}" --plan "${plan}" 2>/dev/null || true
```

Start execution trace for debugging:

```bash
/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local trace start "${phase}" "${plan}" 2>/dev/null || true
```
</step>
...
```bash
AUTONOMY=$(echo "$INIT" | /Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local json get .autonomy --default balanced)
```
...
```bash
REVIEW_CADENCE=$(echo "$INIT" | /Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local json get .review_cadence --default adaptive)
MAX_UNATTENDED_MINUTES_PER_PLAN=$(echo "$INIT" | /Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local json get .max_unattended_minutes_per_plan --default 45)
CHECKPOINT_AFTER_N_TASKS=$(echo "$INIT" | /Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local json get .checkpoint_after_n_tasks --default 3)
CHECKPOINT_AFTER_FIRST_RESULT=$(echo "$INIT" | /Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local json get .checkpoint_after_first_load_bearing_result --default true)
CHECKPOINT_BEFORE_DOWNSTREAM=$(echo "$INIT" | /Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local json get .checkpoint_before_downstream_dependent_tasks --default true)
```
...
2. Per task:
   - `type="auto"`: Execute derivation/calculation/simulation. Verify done criteria including dimensional checks. Commit (see task_commit). Track hash for Summary.
     ```bash
     /Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local observe event task task-complete --phase "${phase}" --plan "${plan}" --data "{\"task\":\"${TASK_NUM}\",\"description\":\"${TASK_DESCRIPTION}\"}" 2>/dev/null || true
     /Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local trace log checkpoint --data "{\"description\":\"Task ${TASK_NUM} done: ${TASK_DESCRIPTION}\"}" 2>/dev/null || true
     ```
     **Required first-result sanity gate:** At the earliest of (a) first quantitative result, (b) first derived core equation, (c) first produced artifact, (d) first benchmark-style comparison, or (e) two completed auto tasks, STOP and check:
     ...
     ```bash
     /Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local observe event execution gate --action enter --phase "${phase}" --plan "${plan}" \
     ...
     ```bash
     /Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local observe event execution gate --action clear --phase "${phase}" --plan "${plan}" \
     ...
     ```bash
     /Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local observe event execution fanout --action lock --phase "${phase}" --plan "${plan}" \
     ...
     /Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local observe event execution fanout --action unlock --phase "${phase}" --plan "${plan}" \
       --data "{\"execution\":{\"checkpoint_reason\":\"pre_fanout\"}}" 2>/dev/null || true
     ```
...
3. Run `<verification>` checks including physics validation (see `execute-plan-validation.md`)
   ```bash
   /Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local observe event verification verification-complete --phase "${phase}" --plan "${plan}" --data "{\"description\":\"${VERIFICATION_RESULT}\"}" 2>/dev/null || true
   /Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local trace log assertion --data "{\"description\":\"Verification: ${VERIFICATION_RESULT}\"}" 2>/dev/null || true
   ```
...
If pausing mid-plan: commit current work, create `.continue-here.md` with full derivation state, recommend `/clear` + `$gpd-resume-work`. See `./.codex/get-physics-done/references/orchestration/context-budget.md` for budget guidelines.
...
```bash
/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local commit "{type}({phase}-{plan}): {description}" --files "${TASK_FILES[@]}"
```
...
/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local json set --file "$COMMITS_FILE" --path "task_${TASK_NUM}" --value "${TASK_COMMIT}"
```
...
if ls "${phase_dir}"/plan-commits-seg-*.json 1>/dev/null 2>&1; then
  /Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local json merge-files --out "${phase_dir}/plan-commits.json" "${phase_dir}"/plan-commits-seg-*.json
...
See `execute-plan-checkpoints.md` for the full checkpoint protocol (display format, types, resume signals) and `./.codex/get-physics-done/references/orchestration/checkpoints.md` for general checkpoint details.
...
Create `${phase}-${plan}-SUMMARY.md` at `${phase_dir}/`. Use `./.codex/get-physics-done/templates/summary.md`.
...
```bash
/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local state advance
/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local state update-progress
/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local state record-metric \
...
/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local state record-session \
  ...
  --resume-file "None"
/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local observe event session continuity-updated --phase "${phase}" --plan "${plan}" --data "{\"stopped_at\":\"Completed ${phase}-${plan}-PLAN.md\",\"resume_file\":\"None\"}" 2>/dev/null || true
```
...
PRE_CHECK=$(/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local pre-commit-check --files "${phase_dir}/${phase}-${plan}-SUMMARY.md" .gpd/STATE.md .gpd/ROADMAP.md 2>&1) || true
echo "$PRE_CHECK"
```
...
```bash
/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local commit "docs(${phase}-${plan}): complete ${PLAN_NAME} plan" --files "${phase_dir}/${phase}-${plan}-SUMMARY.md" .gpd/STATE.md .gpd/ROADMAP.md
```
...
```bash
/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local observe event workflow execute-plan.complete --phase "${phase}" --plan "${plan}" 2>/dev/null || true
```

Stop execution trace (captures event summary):

```bash
/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local trace stop 2>/dev/null || true
```
</step>
