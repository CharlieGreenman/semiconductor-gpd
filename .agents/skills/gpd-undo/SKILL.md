...
2. If the reverted commit advanced the current phase, roll back the position with `gpd state update` / `gpd state patch` rather than direct `apply_patch` so `STATE.md` and `state.json` stay aligned
3. Record the rollback decision:

```bash
/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local state add-decision --phase "undo" --summary "Rolled back: ${TARGET_MSG}" --rationale "User requested undo (checkpoint: ${CHECKPOINT_TAG})"
```
...
/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local commit "docs: update state after undo" --files .gpd/STATE.md .gpd/state.json
```
...
Execute the undo workflow from @./.codex/get-physics-done/workflows/undo.md end-to-end.
