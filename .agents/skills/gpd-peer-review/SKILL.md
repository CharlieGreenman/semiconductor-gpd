...
```bash
CONTEXT=$(/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local --raw validate command-context peer-review "$ARGUMENTS")
if [ $? -ne 0 ]; then
  echo "$CONTEXT"
  exit 1
fi
...
```bash
/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local validate review-preflight peer-review --strict
```
...
```bash
READ_MODEL=$(/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local resolve-model gpd-review-reader)
```
...
task(
  ...
  model="{read_model}",
  readonly=false,
  prompt="First, read ./.codex/agents/gpd-review-reader.md for your role and instructions.
Then read ./.codex/get-physics-done/references/publication/peer-review-panel.md and use its `ClaimIndex` / `StageReviewReport` artifact contract exactly.
...
```bash
LITERATURE_MODEL=$(/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local resolve-model gpd-review-literature)
MATH_MODEL=$(/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local resolve-model gpd-review-math)
```
...
task(
  ...
  readonly=false,
  prompt="First, read ./.codex/agents/gpd-review-literature.md for your role and instructions.
Then read ./.codex/get-physics-done/references/publication/peer-review-panel.md and use its `StageReviewReport` artifact contract exactly.
...
task(
  ...
  model="{math_model}",
  readonly=false,
  prompt="First, read ./.codex/agents/gpd-review-math.md for your role and instructions.
Then read ./.codex/get-physics-done/references/publication/peer-review-panel.md and use its `StageReviewReport` artifact contract exactly.
...
```bash
PHYSICS_MODEL=$(/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local resolve-model gpd-review-physics)
```
...
task(
  ...
  model="{physics_model}",
  readonly=false,
  prompt="First, read ./.codex/agents/gpd-review-physics.md for your role and instructions.
Then read ./.codex/get-physics-done/references/publication/peer-review-panel.md and use its `StageReviewReport` artifact contract exactly.
...
```bash
SIGNIFICANCE_MODEL=$(/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local resolve-model gpd-review-significance)
```
...
task(
  ...
  readonly=false,
  prompt="First, read ./.codex/agents/gpd-review-significance.md for your role and instructions.
Then read ./.codex/get-physics-done/references/publication/peer-review-panel.md and use its `StageReviewReport` artifact contract exactly.
...
Then read ./.codex/get-physics-done/references/publication/peer-review-panel.md, ./.codex/get-physics-done/templates/paper/review-ledger-schema.md, and ./.codex/get-physics-done/templates/paper/referee-decision-schema.md before you write any adjudication artifacts.
...
**Follow the peer-review workflow** from `@./.codex/get-physics-done/workflows/peer-review.md`.
