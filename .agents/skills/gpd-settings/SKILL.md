...
```bash
/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local config ensure-section
INIT=$(/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local init progress --include state,config)
if [ $? -ne 0 ]; then
...
Ask the user once using a single compact prompt block with current values pre-selected:
...
<process>
**Follow the settings workflow** from `@./.codex/get-physics-done/workflows/settings.md`.
