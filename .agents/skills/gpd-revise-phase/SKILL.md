...
```bash
INIT=$(/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local init phase-op "${target_phase}")
if [ $? -ne 0 ]; then
...
```bash
RESULT=$(/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local phase insert "${target_phase}" "${phase_name_with_revised_prefix}")
if [ $? -ne 0 ]; then
...
```bash
/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local state add-decision --phase "${target}" --summary "Superseded Phase ${target} (${name}) with Phase ${replacement_number}" --rationale "${reason}"
```

2. Add a blocker noting the results are under revision:

```bash
/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local state add-blocker --text "Phase ${target} results under revision -- superseded by Phase ${replacement_number}: ${reason}"
```
...
4. Update ROADMAP.md progress table if one exists -- the superseded phase should show status "Superseded" rather than "Complete". Use the apply_patch tool for this ROADMAP.md change (not STATE.md).
...
```bash
PRE_CHECK=$(/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local pre-commit-check --files .gpd/ROADMAP.md .gpd/STATE.md 2>&1) || true
echo "$PRE_CHECK"

/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local commit "docs(phase-${target_padded}): supersede and replace — ${reason}" --files .gpd/
```
</step>
...
Execute the revise-phase workflow from @./.codex/get-physics-done/workflows/revise-phase.md end-to-end.
