...
PHASES=$(/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local phase list)
HIGHEST=$(echo "$PHASES" | /Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local json get .directories[-1] --default "")
if [ -z "$HIGHEST" ]; then
...
```bash
PRE_CHECK=$(/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local pre-commit-check --files .gpd/ROADMAP.md 2>&1) || true
echo "$PRE_CHECK"

/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local commit "docs(roadmap): add gap closure phases {N}-{M}" --files .gpd/ROADMAP.md
```
...
<process>
Execute the plan-milestone-gaps workflow from @./.codex/get-physics-done/workflows/plan-milestone-gaps.md end-to-end.
