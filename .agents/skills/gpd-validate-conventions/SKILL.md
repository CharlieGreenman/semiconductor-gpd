...
```bash
CONTEXT=$(/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local --raw validate command-context validate-conventions "$ARGUMENTS")
if [ $? -ne 0 ]; then
  echo "$CONTEXT"
  exit 1
fi
...
```bash
CONVENTIONS=$(/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local convention list)
```
...
for SUMMARY in .gpd/phases/${PHASE_DIR}/*-SUMMARY.md; do
  /Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local summary-extract "$SUMMARY" --field conventions --field affects
done
```
...
<process>
Execute the validate-conventions workflow from @./.codex/get-physics-done/workflows/validate-conventions.md end-to-end.
