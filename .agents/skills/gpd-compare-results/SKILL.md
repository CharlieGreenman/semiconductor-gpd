...
- ./.codex/get-physics-done/templates/paper/internal-comparison.md -- Template for decisive internal comparisons and verdict ledgers
</required_reading>
...
<process>
```bash
CONTEXT=$(/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local --raw validate command-context compare-results "$ARGUMENTS")
if [ $? -ne 0 ]; then
  echo "$CONTEXT"
  exit 1
fi
...
Execute the compare-results workflow from @./.codex/get-physics-done/workflows/compare-results.md end-to-end.
