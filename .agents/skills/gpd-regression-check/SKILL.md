...
```bash
PROGRESS=$(/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local init progress --include state,roadmap,config)
...
```bash
CONTEXT=$(/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local --raw validate command-context regression-check "${PHASE_ARG:-}")
if [ $? -ne 0 ]; then
  echo "$CONTEXT"
  exit 1
fi
...
```bash
/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local frontmatter get "$VERIF_FILE" --field status
```
...
2. Check each path still exists:
   ```bash
   /Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local verify-path "$ARTIFACT_PATH"
   ```
...
```bash
/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local convention diff <last-verified-phase> <current-phase>
```
</step>
...
```bash
PRE_CHECK=$(/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local pre-commit-check --files ".gpd/REGRESSION-REPORT.md" 2>&1) || true
echo "$PRE_CHECK"

/Users/charlie/.gpd/venv/bin/python -m gpd.runtime_cli --runtime codex --config-dir ./.codex --install-scope local commit "verify: regression check - {V}/{M} verified targets hold, {K} regressions" --files ".gpd/REGRESSION-REPORT.md"
```
...
<process>
Execute the regression-check workflow from @./.codex/get-physics-done/workflows/regression-check.md end-to-end.
